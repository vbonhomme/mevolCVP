#######################
# To cite the mevolCVP function: 
# Evin, A., et al., The long and winding road: identifying pig domestication through molar size and shape, Journal
#of Archaeological Science (2013), http://dx.doi.org/10.1016/j.jas.2012.08.005
#######################



########
#mat: A PCA matrix (individuals in rows and variables in columns). 
#group: the grouping factor (a vector or a factor)
#lim: maximum number of components to be included
#nrep: number of resampling. 
#return.matrix, if =TRUE, the matrix used to draw the graphics is returned.
#minInd, the minimum number of specimens to be included in the analyses. By default the number of specimens in the smallest group.
#########


mevolCVP<-function(mat=mat, group=group, lim=30, nrep=100, return.matrix=FALSE, minInd= NULL, print.legend=TRUE){
  library(MASS)
  mat<-as.matrix(mat)
  ...
  a<-table(group); print(paste("The analyses are done with", ngroup, "groups")); print(a)
  ...
  for (j in 1:nrep){ 
    ...
    if (ncol(mat) == 1){ 
      #balanced samples
      z <- lda(matbalanced, gpbalanced, CV= TRUE)
      tablez <- table(z$class, gpbalanced)
      CVbalanced[j]<-100*sum(diag(tablez))/sum(tablez)
      #balanced randomized samples
      z <- lda(matbalanced, gpbalancedrandom, CV= TRUE) 
      tablez <- table(z$class, gpbalancedrandom)
      CVbalancedrandom[j]<-100*sum(diag(tablez))/sum(tablez)
      #un balanced randomized samples
      z <- lda(as.matrix(mat), gprandom, CV= TRUE)
      tablez <- table(z$class, gprandom)
  ...
  if (ncol(mat) != 1){  
    ...
    if (print.legend==TRUE) legend(2,20,c("CVoriginal", "CVbalanced","CVrandom", "CVbalancedrandom" ),col=c("red","orange", "green", "yellow"),lty=1,lwd=2, bty="o")
  }
  ...
  if (ncol(mat) == 1){  
    ...
    axis(1, c(1,2),labels=c("a", "b")); axis(2); box()
