###pldam allows to perform predictive discriminant analyses based on balanced samples.
# mat= matrix with the 'known' data (usually PC scores, but works also with univariate data)
# group= grouping factor of the 'known' data
# mattoid= matrix with the data to be predicted
# codetoid= label of the specimens to predict
# limproba= a limit for the posterior probability to be concidered
# limCVP= a limit for the cross validation % of the discriminate analysis to be concidered
# nrep= number of replicates
# minInd= the number of specimens to include for the balanced design if smaller than the smallest of the "known" groups


#$matplda provide a summary of the identification

pldam<-function(mat=mat, group=group, mattoid=mattoid, codetoid=codetoid, ncpts=20,limproba=0, limCVP=FALSE, nrep=100, return.matrix=TRUE, minInd= NULL){
	library(MASS)
	mat<-as.matrix(mat)
	...
	for (k in 1:nrep){	
		...
		z <- lda(matr, gpbalanced, CV=TRUE); a<-table(z$class, gpbalanced)
